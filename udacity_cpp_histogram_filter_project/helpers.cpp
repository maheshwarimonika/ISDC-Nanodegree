...
vector < vector <float> > blur(vector < vector < float> > grid, float blurring) {
	...
	float c_prob = 1.0 - blurring;
	float co_prob = blurring / 12.0;
	float a_prob = blurring / 6.0;


	vector< vector<float> > window {{co_prob, a_prob, co_prob},
					{a_prob, c_prob, a_prob},
					{co_prob, a_prob, co_prob}
					};
